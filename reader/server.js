...
function installPythonDeps() {
  ...
  try {
    run(pipCmd + " install --upgrade pip");
  } catch (e) {
    log("Не удалось обновить pip (это не критично), продолжаем...");
  }

  run(pipCmd + ' install -r "' + REQUIREMENTS + '"');
  log("Python-зависимости установлены");
}

function predownloadModels() {
  log("Предзагрузка TTS-моделей (первый запуск)...");
...
function startPythonServer() {
  return new Promise((resolve, reject) => {
    log("Запускаю Python TTS на порту " + PYTHON_PORT + "...");
    ...
    const onData = (data) => {
      ...
      if (!ready && (text.includes("Uvicorn running") || text.includes("Application startup complete"))) {
        ready = true;
        resolve();
      }
    };
    ...
    pythonProcess.on("error", (err) => {
      log("Ошибка запуска Python: " + err.message);
      reject(err);
    });
    ...
    setTimeout(() => {
      if (!ready) {
        log("Python долго не отвечает, но продолжаем...");
        ready = true;
        resolve();
      }
    }, 120000);
...
function startExpress() {
  ...
  app.listen(NODE_PORT, () => {
    log("AlexandriaLib: http://localhost:" + NODE_PORT);
    log("TTS проксируется на Python :" + PYTHON_PORT);
  });
}
